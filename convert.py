...
def process_folder(folder_path):
    ...
    for file_name in os.listdir(folder_path):
        if file_name.endswith('.xhtml'):
            shutil.move(os.path.join(folder_path, file_name), os.path.join(text_folder_path, file_name))
...
def select_folder():
    ...
    if base_folder_path:
        if find_and_process_folder(base_folder_path, target_folder_name):
            messagebox.showinfo("完成", "批处理任务完成。")
        else:
            messagebox.showinfo("警告", "未找到名为 {} 的文件夹。".format(target_folder_name))
    else:
...
btn_select_folder = tk.Button(root, text="选择包含资料夹的最上层资料夹", command=select_folder)
