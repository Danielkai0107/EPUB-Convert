...
def process_folder(folder_path):
    ...
    # 複製 ./Misc 資料夾到處理目標資料夾
    misc_src_folder = resource_path('Misc')
    ...
    if file_paths:
        ...
        for file_path in file_paths:
            ...
            if find_and_process_folder(extracted_folder_path, target_folder_name):
                ...
                # 將解壓縮的資料夾移動到 convert 資料夾中
                convert_folder_path = os.path.join(extract_to, "EPUB轉換")
                if not os.path.exists(convert_folder_path):
                    os.makedirs(convert_folder_path)
                shutil.move(extracted_folder_path, os.path.join(convert_folder_path, extracted_folder_name))
            else:
