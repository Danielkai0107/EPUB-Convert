        ...
        if obs_folder:
            ...
            if html_files and os.path.exists(images_folder):
                ...
                if os.path.exists(styles_folder):
                    ...
                    messagebox.showinfo("成功", "CSS 文件已成功插入到 XHTML 文件中。")
                else:
                    messagebox.showwarning("警告", "在 OEBPS 資料夾中找不到 Styles 資料夾。")
            else:
                messagebox.showerror("錯誤", "資料夾結構不正確，請確認 OEBPS 和 Images 資料夾存在。")
        else:
            messagebox.showerror("錯誤", "未找到 OEBPS 資料夾。")

# 創建 Tkinter 主窗口
root = tk.Tk()
root.title("圖片重新命名與 CSS 插入")
